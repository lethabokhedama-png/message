...
function mapUserForList(user, onlineUsers, lastSeenAt) {
...
function ChatSidebar() {
  ...
  const conversationUsers = conversations.map((user) =>
    mapUserForList(user, onlineUsers, lastSeenAt),
  );
  const allUsers = users.map((user) => mapUserForList(user, onlineUsers, lastSeenAt));
  ...
  return (
    <aside
      className={`flex w-full shrink-0 flex-col overflow-hidden border-r border-border lg:w-72 ${
    ...
    >
      <div className="shrink-0 border-b border-border px-2 pb-2 pt-2.5 sm:px-3 sm:pt-3">
        <div className="flex items-center gap-2 px-0.5 sm:gap-2.5 sm:px-1">
          <AppLogo
            size={32}
            className="size-8 shrink-0 rounded-[var(--radius-tile)] sm:size-8.5"
            alt=""
          />
      ...
      <Tabs
        selectedKey={sidebarTab === "settings" ? "chats" : sidebarTab}
      ...
      >
        ...
        <Tabs.ListContainer className="shrink-0 border-b border-border px-2 pb-2 pt-1">
          <Tabs.List className="w-full gap-0.5">
            <Tabs.Tab id="chats" className="pressable flex-1 justify-center gap-1.5">
              <MessageSquareIcon className="size-3.5 opacity-80" aria-hidden />
              Chats
            </Tabs.Tab>
            <Tabs.Tab id="groups" className="pressable flex-1 justify-center gap-1.5">
        ...
        <Tabs.Panel id="chats" className="flex-1 overflow-x-hidden overflow-y-auto outline-none">
          {filteredConversations.length === 0 && filteredUsers.length === 0 ? (
            ...
            <>
              ...
              {filteredUsers
                .filter((u) => !filteredConversations.some((c) => c.id === u.id))
                .map((user) => (
                  <ConversationRow
